        ...
        if len(self.queue) == 0:
            if len(self.auth) == 0:
                for i in self.fetched:
                    if i in c.incomingURLs.keys():
                        self.auth = self.auth + [len(c.incomingURLs[i]) + 1]
                    else:
                        self.auth = self.auth + [1]
                self.auth = [float(i) / sum(self.auth) for i in self.auth]
            return numpy.random.choice(list(self.fetched), p = self.auth)
        else:
            wyn = self.queue[-1]
            self.queue = self.queue[:-1]
            self.fetched.add(wyn)
...
class Container:
    def __init__(self):
        ...
        self.generatePolicy = LIFO_Policy(queue = self.seedURLs)
