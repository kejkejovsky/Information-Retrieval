...
public class MovieReviewStatictics
{
    ...
    private void processFile(String text)
    {
        ...
        SentenceDetectorME modelDetect = new SentenceDetectorME(_sentenceModel);
        String[] sentence = modelDetect.sentDetect(text);
        noSentences = sentence.length;

        // TODO derive tokens and POS tags from text
        // (update noTokens and _totalTokensCount)
        TokenizerME modelTok = new TokenizerME(_tokenizerModel);
        String[] tokens = modelTok.tokenize(text);
        noTokens = tokens.length;
        POSTaggerME modelPos = new POSTaggerME(_posModel);
        String[] tag = modelPos.tag(tokens);
        ...
        String[] lemmatize = _lemmatizer.lemmatize(tokens, tag);
        for(int i = 0; i<lemmatize.length; i++) {
            if(lemmatize[i].equals("O"))
                continue;
            if(Arrays.asList(lemmatize).indexOf(lemmatize[i]) == i)
                noWords++;
        }

        // TODO derive people, locations, organisations (use tokens),
        // (update people, locations, organisations lists).
        NameFinderME name = new NameFinderME(_peopleModel);
        people = name.find(tokens);
        ...
        for(String t : tag){
            if(t.charAt(0) == 'V'){
                ...
                _nounCount++;
            }else if(t.charAt(0) == 'J'){
                _adjectiveCount++;
            }else if(t.charAt(0) == 'R'){
                _adverbCount++;
            }else if(t.equals("MD")){
                _verbCount++;
            }else if(t.equals("WRB")){
                _adverbCount++;
            }
        }
        // ------------------------------------------------------------------
